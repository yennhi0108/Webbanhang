...
class CreateBookSanphamTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('book_sanpham', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            ...
            $table->foreign('l_ma') //cột khóa ngoại là cột `l_ma` trong table `sanpham`
                ->references('l_ma')->on('book_loai') //cột sẽ tham chiếu đến là cột `l_ma` trong table `loai`
                ->onDelete('CASCADE')
                ->onUpdate('CASCADE');
        });
        DB::statement("ALTER TABLE `book_sanpham` comment 'Sản phẩm # Sản phẩm: truyện, tiểu thuyết, ca dao tục ngữ, ...'");
    }
    ...
    {
        Schema::dropIfExists('book_sanpham');
    }
}
